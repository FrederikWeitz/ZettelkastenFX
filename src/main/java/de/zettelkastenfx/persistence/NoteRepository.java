...
public class NoteRepository {
  ...
  /**
   * Erstellt ein PreparedStatement nach zentraler Uebersetzung der aktiven Zetteltabelle.
   *
   * @param connection Datenbankverbindung
   * @param sql SQL mit fachlichen Altbezeichnern
  ...
  private PreparedStatement prepareStatement(Connection connection, String sql) throws SQLException {
    return connection.prepareStatement(NoteStorageConfig.rewriteSql(sql));
  }

  /**
   * Erstellt ein PreparedStatement mit Auto-Key-Option nach zentraler Zetteltabellen-Uebersetzung.
   *
   * @param connection Datenbankverbindung
   * @param sql SQL mit fachlichen Altbezeichnern
   * @param autoGeneratedKeys Auto-Key-Option
   * @return vorbereitetes Statement
   * @throws SQLException bei JDBC-Fehlern
   */
  private PreparedStatement prepareStatement(Connection connection, String sql, int autoGeneratedKeys) throws SQLException {
    return connection.prepareStatement(NoteStorageConfig.rewriteSql(sql), autoGeneratedKeys);
  }

  /**
   * Liest den Zettelinhalt unabhaengig davon, ob die aktive Spalte BLOB oder TEXT ist.
   *
   * @param rs Ergebniszeile
   * @return Inhalt als interne Byte-Repräsentation
   * @throws SQLException bei JDBC-Fehlern
   */
  private byte[] readContent(ResultSet rs) throws SQLException {
    if (!NoteStorageConfig.CONTENT_IS_TEXT) {
      return rs.getBytes("content_blob");
    }
    String value;
    try {
      value = rs.getString(NoteStorageConfig.CONTENT_COLUMN);
    } catch (SQLException ex) {
      value = rs.getString("content_blob");
    }
    return value == null ? new byte[0] : value.getBytes(StandardCharsets.UTF_8);
  }

  /**
   * Bindet den Zettelinhalt passend zum aktiven Speichertyp.
   *
   * @param ps Statement
   ...
   * @throws SQLException bei JDBC-Fehlern
   */
  private void bindContent(PreparedStatement ps, int index, Note note) throws SQLException {
    if (NoteStorageConfig.CONTENT_IS_TEXT) {
      ps.setString(index, new String(note.getBodyBlob(), StandardCharsets.UTF_8));
    } else {
      ps.setBytes(index, note.getBodyBlob());
    }
  }

  public int countNotes() {
    try (Connection c = ds.getConnection();
         PreparedStatement ps = prepareStatement(c, "SELECT COUNT(*) FROM notes");
  ...
  public int maxNoteId() {
    try (Connection c = ds.getConnection();
         PreparedStatement ps = prepareStatement(c, "SELECT COALESCE(MAX(id), 0) FROM notes");
         ResultSet rs = ps.executeQuery()) {
      return rs.next() ? rs.getInt(1) : 0;
    } catch (SQLException e) {
  ...
  public Optional<Note> load(int id) {
    ...
    try (Connection c = ds.getConnection();
      ...
      try (ResultSet rs = ps.executeQuery()) {
        ...
        note.setBodyBlob(readContent(rs));
  ...
  private int insert(Note note) {
    ...
    try (Connection c = ds.getConnection()) {
      ...
      try (PreparedStatement ps = prepareStatement(c, sql, Statement.RETURN_GENERATED_KEYS)) {
        ps.setString(1, note.getTitle());
        bindContent(ps, 2, note);
  ...
  private void setKeywordsForNote(Connection c, int noteId, List<String> keywords) throws SQLException {
    ...
    try (PreparedStatement del = prepareStatement(c, "DELETE FROM note_keywords WHERE note_id = ?")) {
      del.setInt(1, noteId);
      del.executeUpdate();
    }

    try (PreparedStatement insKw = prepareStatement(c, "INSERT OR IGNORE INTO keywords(keyword) VALUES(?)")) {
    ...
    String joinSql = """
        ...
        """;
    try (PreparedStatement join = prepareStatement(c, joinSql)) {
      for (String k : normalized) {
  ...
  public void mergeKeywords(int keepKeywordId, int removeKeywordId) {
    ...
    try (Connection c = ds.getConnection()) {
      c.setAutoCommit(false);
      try (PreparedStatement copyPs = prepareStatement(c, copySql);
           PreparedStatement deleteJoinPs = prepareStatement(c, deleteJoinSql);
           PreparedStatement deleteKeywordPs = prepareStatement(c, deleteKeywordSql)) {
  ...
  public boolean deleteNoteIfItIsLast(int noteId) {
    ...
    try (Connection c = ds.getConnection()) {
      ...
      try {
        ...
        try (PreparedStatement ps = prepareStatement(c, existsHigherSql)) {
          ps.setInt(1, noteId);
          try (ResultSet rs = ps.executeQuery()) {
        ...
        try (PreparedStatement ps = prepareStatement(c, deleteNoteSql)) {
          ps.setInt(1, noteId);
  ...
  public boolean reverseSuccessorLink(int fromNoteId, int toNoteId) {
    ...
    try (Connection c = ds.getConnection()) {
      ...
      try {
        ...
        try (PreparedStatement ps = prepareStatement(c, selectSql)) {
          ps.setInt(1, fromNoteId);
          ps.setInt(2, toNoteId);
        ...
        try (PreparedStatement ps = prepareStatement(c, deleteSql)) {
          ps.setInt(1, fromNoteId);
          ps.setInt(2, toNoteId);
          ps.executeUpdate();
        }

        try (PreparedStatement ps = prepareStatement(c, insertSql)) {
          ps.setInt(1, toNoteId);
